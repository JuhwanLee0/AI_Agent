...
class CompanyOrchestrator:
    ...
    def load_system_prompt(self, agent_name: str) -> str:
        ...
        # [1. 전사 공통 핵심 지침 (경량화 & 실질 업무 중심)]
        common_handoff = (
            "- **인수인계 원칙**: 1. 본인의 구체적 작업 및 산출물 경로 요약 2. YAGNI 및 탈-AI 티 준수 3. 다음 담당자(@에이전트명) 태그 및 명확한 작업 지시\n"
            "- **산출물 격리**: `projects/<프로젝트명>/` 에 실제 물리 파일로 저장\n"
            "- **절대 금기**: 빈 테이블(| 구분 | 내용 |), 더미 텍스트, 가짜 이름(@홍길동 등) 사용 절대 금지. 오직 등록된 정규 에이전트명(@개발_사원A, @개발_사원B 등)만 태그하십시오.\n"
            "- **도구 실행**: 실제 파일 생성 시 `[TOOL:write_file path=\"...\" content=\"...\"]` 또는 ```html 코드블록 사용"
        )
...
"""
    ...
    def clean_llm_response(self, text: str) -> str:
        ...
        text = re.sub(r'@(?:프론트엔드팀_)?(?:다음담당자:?\s*@?)?홍길동[^\s]*', '', text)
        text = re.sub(r'\|\s*구분\s*\|\s*내용\s*\|', '', text)
        text = re.sub(r'\|\s*-+\s*\|\s*-+\s*\|', '', text)
        text = re.sub(r'인수인계\s*4대\s*양식', '', text)
